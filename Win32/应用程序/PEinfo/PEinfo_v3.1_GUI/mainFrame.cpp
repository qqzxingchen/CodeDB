...
HANDLE	tReadHandle[CHILDWINDOWNUM];	//线程句柄
				   ...
				   int iCmdShow)
{
	...
	for(int i=0;i<CHILDWINDOWNUM;i++)
		tReadHandle[i]=INVALID_HANDLE_VALUE;
		...
		case IDM_OPENFILE:
			...
			if(GetOpenFileName(&ofn)==TRUE)
			{
				if(peBase.Init(ofn.lpstrFile,&(ofn.lpstrFile)[ofn.nFileExtension]))
				{
					...
					tReadHandle[XC_IMPORT_DESCRIPTOR]=CreateThread(NULL,0,StartImportThread,(LPVOID)&peBase,0,NULL);
...
LRESULT	CALLBACK InfoWndProc(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam)
{
	...
	case WM_PAINT:
		...
		if(pCWindInfo->ID==XC_IMPORT_DESCRIPTOR)
		{
			PatBlt(hdcMem[pCWindInfo->ID],0,0,
				GetSystemMetrics(SM_CXSCREEN),GetSystemMetrics(SM_CYSCREEN)*pCWindInfo->drawBoardSize,
					BLACK_BRUSH);
			if(	tReadHandle[XC_IMPORT_DESCRIPTOR]!=INVALID_HANDLE_VALUE && WaitForSingleObject(tReadHandle[XC_IMPORT_DESCRIPTOR],0)!=WAIT_TIMEOUT)
			{
				...
				{	
					...
					DrawText(hdcMem[pCWindInfo->ID],((PONEIMPORTLINE)(peImport.pAllImport[i]))->ID,-1,&rect,DT_SINGLELINE|DT_LEFT);
					rect.left+=150;
					DrawText(hdcMem[pCWindInfo->ID],((PONEIMPORTLINE)(peImport.pAllImport[i]))->Value,-1,&rect,DT_SINGLELINE|DT_LEFT);
					rect.left-=150;
				...
				DrawText(hdcMem[pCWindInfo->ID],"导入表结束",-1,&rect,DT_SINGLELINE|DT_LEFT);
			}
	...
	case WM_SIZE:
		...
		if(pCWindInfo->ID==XC_IMPORT_DESCRIPTOR)
			pCWindInfo->scrollInfo.nMax=XC_SCROLL_PAGE*30-1;
...
DWORD WINAPI StartImportThread( LPVOID point )
{
	peImport.Init(peBase);

	InvalidateRect(hChildWindow[XC_IMPORT_DESCRIPTOR],NULL,TRUE);
	return 0;
}
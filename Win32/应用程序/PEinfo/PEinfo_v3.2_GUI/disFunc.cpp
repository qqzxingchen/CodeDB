...
DWORD WINAPI StartReadThread( LPVOID point )
{
	...
	InvalidateRect(hChildWindow[XC_EXPORT_DESCRIPTOR],NULL,TRUE);
	return 0;
}
...
void DisplayImportInfo( HDC hdc,PRECT pRect,PCHILDWININFO pCWindInfo )
{
	...
	if(	tReadHandle!=INVALID_HANDLE_VALUE && WaitForSingleObject(tReadHandle,0)!=WAIT_TIMEOUT)
	{
		...
		{	
			...
			pRect->left+=150;
			DrawText(hdc,((PONEIMPORTLINE)(peImport.pAllImport[i]))->Value,-1,pRect,DT_SINGLELINE|DT_LEFT);
			pRect->left-=150;
	...
	if(	tReadHandle!=INVALID_HANDLE_VALUE && WaitForSingleObject(tReadHandle,0)!=WAIT_TIMEOUT)
	{
		...
		{	
			...
			else
			{
				...
				DrawText(hdc,((PONEEXPORTLINE)(peExport.pAllExport[i-6]))->name,-1,pRect,DT_SINGLELINE|DT_LEFT);

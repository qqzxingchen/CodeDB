...
typedef struct XC_DIALOGBOXHEADER{
	DWORD lStyle;
	DWORD lExtendedStyle;
	WORD  NumberOfItems;
	XC_PEINFO_rect rect;	//该结构体不完整，因为结构体内有些项是变长存储的
}XC_DIALOGBOXHEADER;

typedef struct XC_DIALOGCONTROLDATA{
	DWORD lStyle;
	DWORD lExtendedStyle;
	XC_PEINFO_rect rect;	//该结构体不完整，因为结构体内有些项是变长存储的
	WORD  wID;
}XC_DIALOGCONTROLDATA;

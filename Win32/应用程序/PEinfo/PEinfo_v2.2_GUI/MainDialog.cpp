...
LRESULT	CALLBACK InfoWndProc	(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam)
{
	...
	case XC_CREATEBITMAP:
		...
		hBitmap[(int)wParam] = CreateCompatibleBitmap(hdc,
	...
	case WM_PAINT:
		...
		BitBlt(hdc, 0, 0, peInfo.windowSize[i].x,peInfo.windowSize[i].y, hdcMem[i], 
			0, (int)(((float)peInfo.scrollInfo[i].nPos/peInfo.scrollInfo[i].nMax)*peInfo.windowSize[i].y*pageNum[i]), 
			SRCCOPY); 
	...
	case WM_SIZE:
		...
		peInfo.scrollInfo[i].nMax=XC_SCROLL_PAGE*pageNum[i]-1;

...
HWND	hwndDialog;						//子对话框句柄（用来实时转换RVA到FOA）
		...
		case IDM_OPENFILE:
			...
			if(GetOpenFileName(&ofn)==TRUE)
			{
				if(peBase.Init(ofn.lpstrFile,&(ofn.lpstrFile)[ofn.nFileExtension]))
				{
					...
					SetTimer(hChildWindow[XC_DELAY_IMPORT_DESCRIPTOR],XC_REDRAWTIMER,XC_REDRAWTIME,NULL);
				}
				else
				{
...
LRESULT	CALLBACK InfoWndProc(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		case XC_DELAY_IMPORT_DESCRIPTOR:	pCWindInfo->drawBoardSize=1;	break;
		}
	...
	case WM_PAINT:
		...
		else if(pCWindInfo->ID==XC_DELAY_IMPORT_DESCRIPTOR)
		{
			DisplayDelayImportInfo(hdcMem[pCWindInfo->ID],&rect,pCWindInfo);
			BitBlt(hdc,0,0,pCWindInfo->windowSize.x,pCWindInfo->windowSize.y,hdcMem[pCWindInfo->ID],0,0,SRCCOPY);	
		}
		else
		{
	...
	case WM_SIZE:
		...
		if(pCWindInfo->ID==XC_EXPORT_DESCRIPTOR)
			pCWindInfo->scrollInfo.nMax=XC_SCROLL_PAGE*300-1;
		if(pCWindInfo->ID==XC_DELAY_IMPORT_DESCRIPTOR)
			pCWindInfo->scrollInfo.nMax=XC_SCROLL_PAGE*300-1;
							...
							)
{
	...
	case WM_COMMAND:
		switch(LOWORD(wParam)){
		case IDC_RVA:
 			GetDlgItemText(hwndDlg,IDC_RVA,temp,sizeof(temp));
 			if(strlen(temp)>0 && strlen(temp)<=8)
			{
				if(getValueFromString(temp,&value))
				{
					if(peBase.RvaToFoa(value)!=0)
					{
						wsprintf(temp,"%X",peBase.RvaToFoa(value));
						SetDlgItemText(hwndDlg,IDC_FOA,temp);
					}
					else
					{
						SetDlgItemText(hwndDlg,IDC_FOA,TEXT("INVALID ADDRESS"));
					}
				}
				else
				{
					SetDlgItemText(hwndDlg,IDC_FOA,TEXT("ERROR"));
				}
			}
			else
			{
				SetDlgItemText(hwndDlg,IDC_FOA,TEXT("NULL"));
			}
...
bool getValueFromString(char *temp,DWORD *value)
{
	int i;
	*value=0;
	for(i=0;i<(int)strlen(temp) && i<8;i++)
	{

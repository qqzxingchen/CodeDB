...
extern PEDelayImport peDelayImport;
...
void DisplayDelayImportInfo( HDC hdc,PRECT pRect,PCHILDWININFO pCWindInfo )
{
	...
	if(	tReadHandle!=INVALID_HANDLE_VALUE && WaitForSingleObject(tReadHandle,0)!=WAIT_TIMEOUT)
	{
		...
		KillTimer(hChildWindow[XC_DELAY_IMPORT_DESCRIPTOR],XC_REDRAWTIMER);
		if(peDelayImport.num==0)
		{
			DrawText(hdc,"该文件无延迟导入表",-1,pRect,DT_SINGLELINE|DT_LEFT);
			return ;
		}
		
		for(i=(int)(peDelayImport.num)*pCWindInfo->scrollInfo.nPos/pCWindInfo->scrollInfo.nMax;
		((i<(int)(peDelayImport.num)*pCWindInfo->scrollInfo.nPos/pCWindInfo->scrollInfo.nMax
			+(int)GetSystemMetrics(SM_CYSCREEN)/XC_ONELINEHEIGHT) && (i<(int)peDelayImport.num));i++)
		{	
			if(strncmp((char*)(peDelayImport.pAllDelayImport[i]),"DLL Name:",9)!=0)
			{
				pRect->left+=20;
				DrawText(hdc,(char*)(peDelayImport.pAllDelayImport[i]),-1,pRect,DT_SINGLELINE|DT_LEFT);
				pRect->left-=20;
			}
			else
			{
